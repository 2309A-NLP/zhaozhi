...
class MysqlRepository:
    ...
    def save_document_chunk(self, chunk: Dict[str, Any]) -> None:
        sql = """
        INSERT INTO document_chunks (chunk_id, document_id, document_name, source_path, chunk_text, page_number, content_type)
    ...
    def list_document_chunks(self, document_id: str = "") -> List[Dict[str, Any]]:
        ...
        if scoped_document_id:
            rows = self._query_all(
                """
                SELECT chunk_id, document_id, document_name, source_path, chunk_text, page_number, content_type
    ...
    def _ensure_database(self) -> None:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    f"CREATE DATABASE IF NOT EXISTS `{self._escape_identifier(self.database)}` "
    ...
    def _ensure_tables(self) -> None:
        ...
        with self._connection() as conn:
            with conn.cursor() as cursor:
                ...
                self._ensure_document_chunk_columns(cursor)
            conn.commit()

    def _ensure_document_chunk_columns(self, cursor) -> None:
        cursor.execute("SHOW COLUMNS FROM document_chunks")
        existing_columns = {str(row["Field"]) for row in cursor.fetchall()}
        if "page_number" not in existing_columns:
            cursor.execute("ALTER TABLE document_chunks ADD COLUMN page_number INT NULL AFTER chunk_text")
        if "content_type" not in existing_columns:
            cursor.execute("ALTER TABLE document_chunks ADD COLUMN content_type VARCHAR(32) NOT NULL DEFAULT 'text' AFTER page_number")
