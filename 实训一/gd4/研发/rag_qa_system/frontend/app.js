...
async function uploadAndIngest() {
  ...
  if (!file.name.toLowerCase().endsWith(".pdf")) {
    resultBox.textContent = "Please choose a PDF file.";
    return;
  }
  if (file.size > MAX_UPLOAD_BYTES) {
    resultBox.textContent = `The selected PDF is too large. Max size is ${Math.round(MAX_UPLOAD_BYTES / 1024 / 1024)} MB.`;
    return;
  }
  try {
