...
def build_api(config: AppConfig | None = None) -> tuple[HttpApi, AppConfig]:
    ...
    answer_service = AnswerService(
        ...
        conversation_max_messages=config.conversation_max_messages,
    )
    knowledge_service = KnowledgeService(
        ...
        ingestor=KnowledgeIngestor(
            parser=PdfParser(
                header_ratio=config.pdf_header_ratio,
                footer_ratio=config.pdf_footer_ratio,
                repeated_line_threshold=config.pdf_repeated_line_threshold,
            ),
